...
class CFEInjector:
    def __init__(self,disassemblyName,skip='0',board=None, createXml=False):
        self.pcInjector = PCinjector(board)
            
        #self.workbook = load_workbook('{}/{}'.format(os.path.dirname(os.path.dirname(__file__)),'randomCFEInjection.xlsx'))

        self.workbook = load_workbook(
            os.path.join(os.path.dirname(os.path.dirname(__file__)), 'randomCFEInjection.xlsx')
        )
        self.cfg = self._createCFG(disassemblyName,skip)
        if createXml:
            title = disassemblyName[0:-7] + ".xml"
            self.XMLCreation(disassemblyName,title,skip )
    ...
    def injectionSuiteInterBlock(self, numberOfCFEs, nameSheet):
        ...
        for i in range(0, numberOfCFEs):
            logging.log(25, "Injecting Inter Block CFE nr %d" % i)
            ...
            while not faultInjected:
                try:
                    ...
                    possNewPC = self._possibleNewPCList(origPC,True,False,True)
                    bitFlippedPC = possNewPC[random.randint(0, len(possNewPC)-1)]
                    logging.info("PC to inject: 0x%X" % bitFlippedPC)
                    self.pcInjector.writeNewPC(bitFlippedPC)
                    sleep(.3)
                    if(self.checkError(0x000)):
                        detected = 1
                        logging.debug("InterBlock detected")
                    elif(self.checkError(0x20e)):
                        hardFault = 1
                        logging.debug("Hardfault detetected")
                    elif(self.checkError(0x278)):
    ...
    def injectionSuiteIntraBlock(self, numberOfCFEs, nameSheet):
        # get Excel worksheet
        ws = self._initExcelSheet(nameSheet, "Intra Block only", numberOfCFEs)
        for i in range(0, numberOfCFEs):
            logging.log(25, "Injecting Intra Block CFE nr %d" % i)
            sleep(random.random()*2)
            detected = 0
            hardFault = 0
            undWR = 0
            ...
            while not faultInjected:
                try:
                    origPC = self.pcInjector.readPC()
                    logging.info("Read PC: 0x%X" % origPC)
                    possNewPC = self._possibleNewPCList(origPC,False,True,True)
                    bitFlippedPC = possNewPC[random.randint(0, len(possNewPC)-1)]
                    logging.info("PC to inject: 0x%X" % bitFlippedPC)
                    self.pcInjector.writeNewPC(bitFlippedPC)
                    sleep(.3)
                    ...
                    #elif self.checkErrorBB(4):
                        logging.debug("IntraBlock undetected, wrong result")
                        undWR = 1
                    else:
                        undCR =1
                        logging.debug("IntraBlock undetected, correct result")
                    faultInjected = True
                    ws = self._writeToExcel(ws, nameSheet,i+3,origPC,bitFlippedPC,detected,hardFault,undWR, undCR)
                except Exception as e:
                    logging.error("Error while injecting CFE\n%s"%e)
    ...
    def injectionSuiteOutFunction(self, numberOfCFEs, nameSheet):
        # get Excel worksheet
        ws = self._initExcelSheet(nameSheet, "Out Function only", numberOfCFEs)
        for i in range(0, numberOfCFEs):
            logging.log(25, "Injecting Out Function CFE nr %d" % i)
            sleep(random.random()*2)
            detected = 0
            hardFault = 0
            undWR = 0
            ...
            while not faultInjected:
                try:
                    origPC = self.pcInjector.readPC()
                    logging.info("Read PC: 0x%X" % origPC)
                    possNewPC = self._possibleNewPCList(origPC,True,True,False)
                    bitFlippedPC = possNewPC[random.randint(0, len(possNewPC)-1)]
                    logging.info("PC to inject: 0x%X" % bitFlippedPC)
                    self.pcInjector.writeNewPC(bitFlippedPC)
                    sleep(.3)
        ...
        for i in range (0, numberOfCFEs):
            ...
            while not faultInjected:
                ...
                try:
                    ...
                    sleep(.3)
                    if(self.checkError(0x27a)):
                        hardFault = 1
                        logging.debug("HardFault")
                    elif(self.checkError(0x2e0)):
                        WR = 1
    ...
    def deterministicCFEsuite(self, nrIntra, nrInter, nrOutFunc):
        if nrIntra != 0:
            logging.log(25, "-------------Starting the IntraBlock injection-------------")
            self.injectionSuiteIntraBlock(nrIntra, "IntraBlockCFE")
            logging.log(25, "-------------Finished the IntraBlock injection-------------")
        if nrInter != 0:
            logging.log(25, "-------------Starting the InterBlock injection-------------")
            self.injectionSuiteInterBlock(nrInter, "InterBlockCFE")
            logging.log(25, "-------------Finished the InterBlock injection-------------")
        if nrOutFunc != 0:
            logging.log(25, "-------------Starting the OutFunction injection-------------")
            self.injectionSuiteOutFunction(nrOutFunc, "OutFunctionCFE")
            logging.log(25, "-------------Finished the OutFunction injection-------------")
        logging.log(25, "##############################################\nFINISHED all Injection\n##############################################")
    ...
    def _keepOutFunction(self, listOfPossibilities):
        logging.debug("Cleaning up all correct PC values")
        validPoss = []
        for i in range(0, len(listOfPossibilities)):
            poss = listOfPossibilities[i]
            if not self._isInCFG(poss):
                validPoss.append(poss)
            else:
                found = False
                for x in range(0, len(self.cfg)):
    ...
    def _possibleNewPCList(self, pc, cleanupIntra, cleanupInter, cleanupWrong):
        validPoss = self._createAllPossibleOneBitFlipResults(pc)
        if cleanupWrong:
            validPoss = self._cleanupWrongPossibilities(validPoss)
            if (cleanupIntra):
            ...
            validPoss = self._cleanupTrueFalseJumpAddresses(pc, validPoss)
        else:
            validPoss = self._keepOutFunction(validPoss)
        return validPoss
    ...
    def _writeToExcel(self, worksheet, namesheet, row, origPC, injPC, detected, hardFault, undWR, undCR):
        ...
        # Save workbook
        self.workbook.save("detCFEInjection.xlsx")
